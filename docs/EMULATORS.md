# Emulators danstick configures

danstick publishes stable virtual gamepads — `danstick Player 1`..`N`, each with a
...
configuration so player N is already bound to `danstick Player N`, and the user
...
`SDL_OpenJoystick`, `SDL_GetNumJoystickAxes|Hats|Buttons` — so danstick's mapping
...
profile alone is not enough — danstick's mapping has to reach it through
...
**Ryujinx could not tell danstick's pads apart, so danstick changed.** It builds
...
was the only thing distinguishing danstick's pads, so four players collapsed to
one id and the binding fell back to SDL connection order.

danstick is the abstraction layer, so this is danstick's problem and not a caveat
...
* **The device line**, `SDL/<n>/<name>`, where `n` counts devices already
  sharing that *name*. Every danstick pad is named for its player, so `n` is
  ...
  The name is the *clone's*, `danstick Player N`, not what SDL's database calls
  the pad behind it. Measured against Dolphin's own controller log
  (`[Logs] CI = True` in `Logger.ini`), which lists `SDL/0/danstick Player 1`
  and `SDL/0/danstick Player 2` beside the grabbed raw pad. A tool that reports
...
and Ryujinx, danstick *will* create them: neither is the whole of Dolphin's
...
block danstick left at another port last time is removed, so the keyboard is
...
danstick serves every seated player's gyro and accelerometer over **DSU**, the
cemuhook UDP protocol, on `127.0.0.1:26760`. Slot N-1 is player N. Every config
danstick writes points at it:
...
| Dolphin | a `danstick` entry in `DSUClient.ini`; nothing is bound, because a GameCube pad has no gyro |
...
enumeration order. DSU addresses controllers by slot, which is exactly danstick's
player number.
...
node, so danstick decodes the IMU block from the reports it already reads.
...
differently from the d-pad on, and danstick writes the matching table.
...
**Turning it off.** `DANSTICK_DSU_PORT=0`. A port already taken by another DSU
server is not fatal: danstick logs it, and the controllers work without motion.

## Why the button tables are constant

They look like they should be per-controller and they are not. For Cemu and
Ryujinx the table maps an emulated button to an **SDL gamepad id**, and danstick's
...
The mapping has to arrive in the environment, so danstick writes
`$XDG_RUNTIME_DIR/danstick/env.sh` on every republish:

```sh
danstick-rs exec -- Cemu       # or: . "$XDG_RUNTIME_DIR/danstick/env.sh"; Cemu
```
...
`rewrite_ares_settings` replaces the `VirtualPadN` blocks danstick manages and
