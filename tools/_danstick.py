"""The few things a diagnostic tool needs that danstick's own binary does not print.

danstick is a Rust binary now, so there is no package to import. These are the
...
def runtime_dir() -> Path:
    return Path(os.environ.get("XDG_RUNTIME_DIR", "/tmp")) / "danstick"


def socket_path() -> Path:
    return runtime_dir() / "danstick.sock"
...
def daemon_pids(runtime: str | None = None) -> list[int]:
    """Pids of danstick daemons serving a given XDG_RUNTIME_DIR.
    ...
    for entry in Path("/proc").iterdir():
        ...
        if len(argv) < 2 or argv[-1] != "serve":
            continue
        if Path(argv[-2]).name not in ("danstick", "danstick-rs"):
            continue
        theirs = "/tmp"
        for item in environ:
