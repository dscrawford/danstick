  ...
  outputs = { self, nixpkgs, flake-utils }:
    (flake-utils.lib.eachDefaultSystem (system:
      let
        ...
        # Test scaffolding only -- danstick itself is a Rust binary. These are
        ...
        # `danstick` and `danstick-rs` as commands inside the dev shell, running
        # the working tree rather than a store copy.
        #
        # packages.danstick already exists, but it bakes in ${./src}: entering
        # the shell and editing a file would leave `danstick list` running the
        ...
        #
        # So these resolve from DANSTICK_DEV_ROOT, which the shellHook pins to
        ...
        # danstick you are running.
        devRoot = ''
          if [ -z "''${DANSTICK_DEV_ROOT:-}" ]; then
            echo "danstick: DANSTICK_DEV_ROOT is unset." >&2
            echo "danstick: this wrapper only works inside the dev shell." >&2
            exit 1
          fi
        '';

        # Builds on first use and after every edit. That is the point --
        # `danstick list` should never be stale -- but it means the first call
        ...
        #
        # DANSTICK_BUILD_ID is deliberately *not* set here. packages.danstick sets
        ...
        devDanstick = pkgs.writeShellScriptBin "danstick" ''
          set -euo pipefail
          ${devRoot}
          exec cargo run --quiet --release \
            --manifest-path "$DANSTICK_DEV_ROOT/rust/Cargo.toml" \
            --bin danstick-rs -- "$@"
        '';
        ...
        danstick-rs = pkgs.rustPlatform.buildRustPackage {
          pname = "danstick-rs";
          version = "0.1.0";
          src = ./rust;
          ...
          # `exec` starts a game in a sandbox where only danstick's pads are
          ...
          postInstall = ''
            wrapProgram $out/bin/danstick-rs \
      ...
      in
      {
        devShells.default = pkgs.mkShell {
          ...
          shellHook = ''
            ...
            echo "danstick dev shell -- these build from $DANSTICK_DEV_ROOT, not a store copy"
            echo "  danstick list          - what is plugged in"
        ...
        # refresh the udev rules. It launches nothing afterwards -- danstick is a
        ...
        # whatever does.
        packages.danstick-start = pkgs.writeShellApplication {
          name = "danstick-start";
          text = ''
            if [ "''${DANSTICK_SKIP_DAEMON_CHECK:-0}" != "1" ]; then
              ${self.packages.${system}.danstick}/bin/danstick ensure-daemon \
                || echo "danstick: continuing without a current daemon" >&2
            fi
            ...
            # *physical* pad as well as danstick's clone, and a wizard that asks
            ...
            # and carry on.
            if [ "''${DANSTICK_SKIP_HIDE:-0}" != "1" ]; then
              if ! sudo -n ${self.packages.${system}.danstick}/bin/danstick hide \
                     >/dev/null 2>&1; then
                echo "danstick: could not refresh the udev hide rules (needs root)." >&2
                echo "danstick: run 'sudo danstick hide' once, or set" >&2
                echo "danstick:   programs.danstick.autoHide = true;" >&2
                echo "danstick: so they are kept current automatically." >&2
              fi
            fi
        ...
        # so a client needs the files even though danstick never renders them.
        packages.icons = pkgs.runCommand "danstick-icons" { } ''
          mkdir -p "$out/share/danstick/icons"
          cp ${./assets/icons}/*.svg "$out/share/danstick/icons/"
        '';

        packages.default = self.packages.${system}.danstick;

        packages.danstick-rs = danstick-rs;
        ...
        # danstick itself. Kept under this name as well as `packages.danstick`
        # so `nix run .#danstick-rs` still works for anyone who scripted it.
        apps.danstick-rs = {
          type = "app";
          program = "${danstick-rs}/bin/danstick-rs";
        };
      ...
      # into the store.
      nixosModules.danstick = { config, lib, pkgs, ... }:
        let
          cfg = config.programs.danstick;
          ...
          # machine with no usable controllers whenever danstick is not running.
          hideRule = spec:
            let
            ...
            assert lib.assertMsg (lib.length parts == 2)
              "programs.danstick.hideDevices: expected \"vvvv:pppp\", got ${spec}";
            ''SUBSYSTEM=="input", ATTRS{idVendor}=="${vendor}", ATTRS{idProduct}=="${product}", ENV{ID_INPUT_JOYSTICK}=""'';
        in
        {
          options.programs.danstick = {
            enable = lib.mkEnableOption "danstick controller assignment";

            package = lib.mkOption {
              type = lib.types.package;
              default = self.packages.${pkgs.system}.danstick;
              defaultText = lib.literalExpression "danstick.packages.\${system}.danstick";
              description = "The danstick package to install.";
            };
            ...
            autoHide = lib.mkOption {
              ...
              description = ''
                ...
                device alongside danstick's clone. The visible symptom is
                ...
                With this on, a oneshot service runs `danstick hide` at boot and
          ...
          config = lib.mkIf cfg.enable {
            ...
            systemd.services.danstick-hide = lib.mkIf cfg.autoHide {
              description = "Refresh danstick's udev rules for the connected pads";
              ...
              serviceConfig = {
                Type = "oneshot";
                ExecStart = "${cfg.package}/bin/danstick hide";
              };
            };

            services.udev.extraRules =
              lib.optionalString (cfg.hideDevices != [ ]) ''
                # Generated by the danstick NixOS module.
                ${lib.concatMapStringsSep "\n" hideRule cfg.hideDevices}
              ''
              + lib.optionalString cfg.autoHide ''
                # Regenerate danstick's hide rules whenever a joypad appears.
                # `danstick hide` is a no-op when the rules already match, so
                # the reload it would otherwise trigger cannot re-enter this.
                SUBSYSTEM=="input", ACTION=="add", ENV{ID_INPUT_JOYSTICK}=="1", TAG+="systemd", ENV{SYSTEMD_WANTS}+="danstick-hide.service"
              '';
          };
        };

      nixosModules.default = self.nixosModules.danstick;
    };
}
