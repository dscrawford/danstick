...
use danstick_core::wire::{self, LineReader};
use danstick_core::{emit, scope};
use danstick_input::clone::{self, IdentityMode, Source};
...
pub enum StartError {
    ...
    #[error("another danstick daemon is already listening on {0}")]
    Occupied(PathBuf),
    #[error("{0} is in the way and is not a socket danstick can remove: {1}")]
...
impl Server {
    ...
    fn start_motion(&mut self) {
        let port = match std::env::var("DANSTICK_DSU_PORT") {
            Ok(value) => match value.trim().parse::<u16>() {
                Ok(port) => port,
                Err(_) => {
                    warn!("DANSTICK_DSU_PORT={value:?} is not a port number; using the default");
                    danstick_core::dsu::PORT
                }
            },
            Err(_) => danstick_core::dsu::PORT,
        };
        if port == 0 {
            debug!("motion server disabled by DANSTICK_DSU_PORT=0");
            return;
        }
    ...
    fn begin_calibration(&mut self, player: u32) {
        ...
        let declared = self
            ...
            .unwrap_or_default();
        let axes: BTreeMap<u16, danstick_core::calibration::Declared> = declared
            .into_iter()
    ...
    fn begin_layout_choice(&mut self, player: u32) {
        ...
        chooser.move_by(danstick_core::layout::index_of(&guess) as i32);
        self.confirm.clear();
    ...
    fn begin_mapping(&mut self, player: u32, layout_id: &str, scope: &str) {
        ...
        let layout = danstick_core::layout::for_icon(&chosen);
    ...
    fn set_icon(&mut self, player: u32, icon: &str) {
        if !danstick_core::icons::known(icon) {
    ...
    fn tick_seating(&mut self) {
        ...
        for pad in taking {
            ...
            self.save_assignments();
            let announced =
                self.controller_event(danstick_core::announce::ACTION_ADDED, player, &pad, "");
    ...
    /// What danstick would tell a consumer about each slot right now.
    fn motion_ports(&self) -> Vec<danstick_core::dsu::Port> {
    ...
    fn set_identity(&mut self, mode: &str) {
        ...
        let Some(wanted) = IdentityMode::parse(mode) else {
            self.broadcast(&events::error(format!(
                "unknown identity {mode:?}; one of mirror, danstick, xbox360, xbox360-numbered"
            )));
            return;
        };
    ...
    fn faces_for(
        &self,
        pad: &Pad,
        mapping: &danstick_core::profile::Mapping,
    ) -> BTreeMap<danstick_core::Control, danstick_core::Control> {
        if self.slot_policy.layout != slots::Layout::Label {
            return BTreeMap::new();
        }
        let layout = if danstick_core::layout::exists(&mapping.layout) {
    ...
    fn listening_for_space(&self) -> bool {
        ...
        self.seating.is_open()
            ...
            && danstick_core::announce::next_player(&self.taken_seats())
    ...
    fn tick_keyboard_hold(&mut self) {
        ...
        if let Some(fraction) = tick.progress {
            ...
            self.broadcast(&events::progress(
                fraction,
                danstick_core::keyboard::SEAT_NAME,
                "",
                Some(player),
            ));
        }
    ...
    fn autosetup_blocked(&self) -> Option<&'static str> {
        if std::env::var(ENV_NO_AUTOSETUP).as_deref() == Ok("1") {
            return Some("disabled by DANSTICK_NO_AUTOSETUP");
        }
        if self.state == STATE_ASSIGNING {
    ...
    fn players_payload(&self) -> Vec<PlayerState> {
        ...
        if let Some(seat) = self.keyboard_seat.filter(|_| self.session.is_none()) {
            players.push(danstick_core::keyboard::seat_state(seat));
        }
    ...
    fn reserved_payload(&self) -> Vec<danstick_core::state::ReservedSeat> {
        self.reserved_nodes
            .iter()
            .filter_map(|(player, node)| {
                let identity = publish::xbox_identity(self.mode, *player)?;
                Some(danstick_core::state::ReservedSeat {
                    player: *player,
                    node: node.clone(),
                    name: danstick_core::emit::virtual_name(*player),
                    guid: danstick_core::emit::virtual_guid(*player, identity),
                })
            })
            .collect()
    }
