...
use danstick_core::capture::{Chooser, MappingRun};
use danstick_core::state::{PlayerState, StateEvent};
...
fn layout_json(layout_id: &str) -> Value {
    serde_json::to_value(danstick_core::layout::get(layout_id)).unwrap_or(Value::Null)
}
...
pub fn tuned(player: u32, signature: &str, tuning: &danstick_core::tuning::Tuning) -> Value {
    json!({
        "event": "tuned",
        "player": player,
        "signature": signature,
