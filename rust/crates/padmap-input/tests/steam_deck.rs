...
//! recording is right, and where they disagree, the disagreement is padmap's
//! and is named here rather than discovered on a Deck.

use std::collections::{BTreeMap, BTreeSet};
use std::sync::OnceLock;

use padmap_core::sdl::{self, AxisSpan};
use padmap_input::fakepad::{Fixture, STEAM_DECK, XBOX_360};
...
fn guessed(fixture: &Fixture) -> BTreeMap<String, String> {
    ...
    padmap_core::guess::guessed_fields(&fixture.key_codes(), &fixture.abs_codes(), Some(&spans))
        .iter()
...
fn sdl_deck() -> &'static BTreeMap<String, String> {
    ...
    FIELDS.get_or_init(|| {
        ...
        let line = padmap_input::sdlprobe::builtin_mapping(&guid)
...
/// Controls padmap has no name for, so it can never bind them.
const BEYOND_PADMAPS_VOCABULARY: [&str; 6] = [
...
#[test]
fn padmap_and_sdl_agree_about_a_deck_except_where_this_test_says_they_do_not() {
    ...
    assert_eq!(
        ...
        "padmap's guess drifted from SDL somewhere new"
    );

    // Everything padmap did not bind is a control it has no word for.
    ...
    assert!(
        unbound
            .iter()
            .all(|name| BEYOND_PADMAPS_VOCABULARY.contains(name)),
        "padmap left something bindable unbound: {unbound:?}"
    );
}
...
fn a_decks_x_and_y_land_on_each_others_buttons() {
    ...
    // padmap's positional reading of the codes comes out the wrong way round.
...
fn a_decks_triggers_are_digital_because_sdls_analogue_answer_sits_on_hat_codes() {
    let ours = guessed(&STEAM_DECK);
    let theirs = sdl_deck();
    // SDL counts ABS_HAT2X/Y among the axes; padmap's axis numbering excludes
