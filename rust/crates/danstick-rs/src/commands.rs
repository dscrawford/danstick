...
use danstick_input::{artefacts, assignments, profiles, runtime};
...
pub fn cmd_setup(wanted: Option<u32>) -> Result<()> {
    use danstick_core::assign::HOLD_SECONDS;
    use danstick_daemon::session::Session;
        ...
        for claim in &ticked.claimed {
            if let Some(pad) = session.pads.get(claim.pad) {
                println!(
                    "  Player {}: {} [{}]",
                    claim.player,
                    danstick_daemon::clean(&pad.name),
                    pad.event()
                );
            }
        }
    ...
    println!("Start the daemon with: danstick run");
    Ok(())
}
...
pub fn cmd_forget(all: bool) -> Result<()> {
    ...
    if database.is_file() {
        println!("\nThe SDL mappings danstick wrote are separate and still present:");
...
pub fn cmd_map(layout_id: Option<String>, which: Option<String>, scope: String) -> Result<()> {
    ...
    if chosen.is_empty() {
        ...
        for layout in danstick_core::layout::all() {
            println!("  {:<10} {}", layout.id, layout.label);
        }
        std::process::exit(1);
    }
    if !danstick_core::layout::exists(&chosen) {
        let known: Vec<&str> = danstick_core::layout::all()
            .iter()
    ...
    let layout = danstick_core::layout::get(&chosen);
    ...
    let name = danstick_daemon::clean(&pad.name);
    println!(
        "\nMapping {name} [{}] as {}{}.",
        pad.event(),
        layout.label,
    ...
    println!("\nRun `danstick run` or `danstick launch` to apply it.");
    Ok(())
}

pub fn cmd_calibrate(force: bool) -> Result<()> {
    use danstick_daemon::calibration::{CalibrationRun, Phase, Step};
    use danstick_daemon::session::Session;
    ...
    println!("Restart the daemon to apply:  danstick ensure-daemon");
    Ok(())
}
...
pub fn cmd_ensure_daemon(check: bool, timeout: f64, lifetime: Lifetime) -> Result<()> {
    let pads = discover().unwrap_or_default();
    let hideable: Vec<danstick_core::hide::Hideable> = pads
        .iter()
        .map(|pad| danstick_core::hide::Hideable {
            name: pad.name.clone(),
            vid: pad.vid,
            pid: pad.pid,
        })
    ...
    let missing = danstick_core::hide::unhidden(&hideable, rules.as_deref());
    if !missing.is_empty() {
        println!(
            "warning: {} adapter(s) danstick republishes are still visible:",
            missing.len()
        );
        for target in &missing {
            println!("  {:04x}:{:04x}  {}", target.vid, target.pid, target.name);
        }
        println!("  regenerate the udev rules with:  danstick hide");
    }
    ...
    let (our_slots, _) = danstick_core::slots::Policy::from_env(|name| std::env::var(name).ok());
...
fn installed_rules() -> Option<String> {
    for path in [
        ...
        "/etc/udev/rules.d/99-danstick.rules",
    ] {
...
pub const RUNTIME_RULES_PATH: &str = "/run/udev/rules.d/99-danstick.rules";
...
pub fn cmd_tune(
    which: Option<String>,
    request: danstick_core::tuning::Request,
    show: bool,
) -> Result<()> {
    let pads = discover()?;
    if pads.is_empty() {
    ...
    if show || request.is_empty() {
        ...
        if request.is_empty() && !show {
            println!("\nNothing to change. See `danstick tune --help`.");
        }
        return Ok(());
    }
...
fn slots_match(state: &serde_json::Value, ours: danstick_core::slots::Policy) -> bool {
    ...
    ours.mode == danstick_core::slots::Mode::OnDemand
