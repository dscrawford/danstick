...
fn every_binding_matches_the_one_cemu_wrote_itself() {
    ...
    let ours = pairs(&cemu::profile(2, GUID, "danstick Player 1"));
...
fn the_uuid_carries_the_guid_with_the_ordinal_prefix() {
    ...
    assert!(xml.contains("<display_name>danstick Player 1</display_name>"));
}
...
#[test]
fn the_filename_counts_from_zero_where_danstick_counts_from_one() {
...
fn motion_arrives_as_a_second_controller_on_the_same_profile() {
    ...
    assert!(xml.contains(&format!("<port>{}</port>", danstick_core::dsu::PORT)));
    assert!(xml.contains(&format!("<ip>{}</ip>", danstick_core::dsu::HOST)));
}

#[test]
fn the_motion_uuid_is_a_bare_slot_number() {
    for player in 1..=4u32 {
        let xml = cemu::profile(player, "0", "danstick");
        assert!(
...
fn a_player_beyond_the_four_dsu_slots_gets_no_motion_entry() {
    ...
    assert!(cemu::profile(4, "0", "danstick").contains("DSUController"));
}
