...
use danstick_core::ryujinx;
use serde_json::{json, Value};

/// danstick's own GUIDs for players 1..4.
const DANSTICK_GUIDS: [&str; 4] = [
...
fn config(player: u32, guid: &str) -> Value {
    ryujinx::input_config(player, guid, "danstick", 0).expect("a config")
}
...
#[test]
fn every_danstick_pad_gets_its_own_id() {
    let ids: Vec<String> = DANSTICK_GUIDS
        .iter()
...
fn a_and_b_are_mirrored_the_way_nintendo_labels_them() {
    let right = &config(1, DANSTICK_GUIDS[0])["right_joycon"];
...
fn an_entry_carries_what_ryujinx_needs_to_read_it() {
    let entry =
        ryujinx::input_config(2, DANSTICK_GUIDS[1], "danstick Player 2", 0).expect("an entry");
    ...
    assert_eq!(entry["name"], "danstick Player 2");
...
fn only_eight_players_exist() {
    assert!(ryujinx::input_config(8, DANSTICK_GUIDS[0], "x", 0).is_some());
    assert!(ryujinx::input_config(9, DANSTICK_GUIDS[0], "x", 0).is_none());
    assert!(ryujinx::input_config(0, DANSTICK_GUIDS[0], "x", 0).is_none());
}

#[test]
fn merging_keeps_entries_danstick_is_not_managing() {
    let existing = json!([
    ...
    let ours =
        vec![ryujinx::input_config(1, DANSTICK_GUIDS[0], "danstick Player 1", 0).expect("entry")];
...
fn merging_into_nothing_is_our_entries_and_a_keyboard() {
    let merged = ryujinx::merge(&Value::Null, vec![config(1, DANSTICK_GUIDS[0])], None);
...
#[test]
fn motion_is_asked_of_danstick_rather_than_of_sdl() {
    let motion = &config(2, DANSTICK_GUIDS[1])["motion"];
    assert_eq!(motion["motion_backend"], "CemuHook");
    assert_eq!(motion["enable_motion"], true);
    assert_eq!(motion["dsu_server_host"], danstick_core::dsu::HOST);
    assert_eq!(motion["dsu_server_port"], danstick_core::dsu::PORT);
}

#[test]
fn player_one_takes_slot_zero() {
    for player in 1..=4u32 {
        let motion = &config(player, DANSTICK_GUIDS[0])["motion"];
...
fn the_backend_name_is_one_ryujinx_will_accept() {
    let backend = config(1, DANSTICK_GUIDS[0])["motion"]["motion_backend"]
        .as_str()
        .expect("a string")
        .to_owned();
...
fn a_player_beyond_the_four_dsu_slots_has_motion_off() {
    for player in 5..=8u32 {
        assert_eq!(
            config(player, DANSTICK_GUIDS[0])["motion"]["enable_motion"],
            false,
            "player {player}"
        );
    }
    assert_eq!(
        config(4, DANSTICK_GUIDS[0])["motion"]["enable_motion"],
        true
    );
}
