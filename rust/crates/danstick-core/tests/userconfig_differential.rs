...
fn line_endings_and_spacing_survive_a_change() {
    ...
    let spaced =
        userconfig::clean("  input_player1_reserved_device  =  \"danstick Player 1\"   \n");
    assert!(spaced
        .text
...
fn an_unquoted_line_stays_unquoted_unless_it_cannot() {
    ...
    let cleaned = userconfig::clean("input_player1_reserved_device = danstickPlayer\n");
    assert!(cleaned.changes.is_empty(), "not a virtual pad's name");
    let cleaned = userconfig::clean("input_player1_reserved_device = \"danstick Player 1\"\n");
...
fn a_latin_one_path_survives_beside_a_line_that_is_rewritten() {
    ...
    input.extend_from_slice(b"input_player1_reserved_device = \"danstick Player 1\"\n");
    let (changes, out) = userconfig::clean_bytes(&input);
