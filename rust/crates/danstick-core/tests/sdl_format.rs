...
use danstick_core::control::{Control, CANONICAL_ORDER};
use danstick_core::fields::Fields;
use danstick_core::sdl::{
...
// The name SDL saw when it wrote REAL_GUID, before the project was renamed.
...
fn every_forbidden_character_leaves_exactly_one_physical_line_with_the_right_field_count() {
    ...
    for bad in FORBIDDEN {
        ...
        assert_eq!(
            ...
            1,
            "{bad:?} split the line for danstick's own rewriter, which would then \
...
fn the_name_is_returned_verbatim_and_is_not_lowercased_or_trimmed() {
    let (_, name, _) =
        parse_line(&format!("{REAL_GUID}, danstick Player 1 ,a:b0,")).expect("parse");
    assert_eq!(name, " danstick Player 1 ");
}
