...
/// Player -> pad index, for the players danstick is actually binding.
...
pub fn reservation_config(players: &[u32], virtual_name: impl Fn(u32) -> String) -> String {
    ...
    let lines = [
        "# Generated by danstick. Pass to RetroArch with --appendconfig.",
...
pub fn derive_profile(
    ...
    let mut out = vec![
        format!("# Generated by danstick for {name}."),
        format!(
...
pub struct LaunchFacts {
    ...
    /// Path to danstick's autoconfig profiles.
...
pub fn launch_config(
    ...
    let mut lines: Vec<String> = vec![
        "# Generated by danstick. Passed to RetroArch with --appendconfig.".to_owned(),
    ...
    if !managed.is_empty() && facts.all_calibrated {
        lines.push(String::new());
        lines.push("# Every managed pad is calibrated, so danstick is already".to_owned());
    ...
    lines.push(String::new());
    lines.push("# Autoconfig comes from danstick's own directory, not".to_owned());
    ...
    if facts.verbose {
        lines.push(String::new());
        lines.push("# Diagnostics, enabled by `danstick launch --log`.".to_owned());
