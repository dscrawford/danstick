...
/// Appears as `DSUClient/<n>/danstick` in Dolphin.
pub const DSU_DESCRIPTION: &str = "danstick";
...
mod tests {
    ...
    fn a_section_is_what_dolphin_itself_wrote() {
        let text = section(1, "SDL/0/danstick Player 1");
    ...
    #[test]
    fn a_danstick_pad_is_always_slot_zero() {
        assert_eq!(device("danstick Player 3"), "SDL/0/danstick Player 3");
    }
    ...
    fn every_pad_section_is_replaced_and_everything_else_is_kept() {
        let existing = "[Core]\nSIDevice0 = 6\n[GCPad1]\nDevice = SDL/0/old\nButtons/A = `x`\n\
                        [GCPad4]\nDevice = SDL/0/gone\n[DSUClient]\nServer = 127.0.0.1\n";
        let body = section(1, "SDL/0/danstick Player 1");
        ...
        assert!(out.contains("Device = SDL/0/danstick Player 1"), "{out}");
    }
    ...
    fn every_wiimote_section_is_replaced_and_everything_else_is_kept() {
        ...
        assert!(
            ...
            "a real remote's pairing outlived danstick's own section: {out}"
        );
        assert!(!out.contains("SDL/0/gone"), "{out}");
    ...
    fn writing_into_nothing_at_all_still_produces_a_file() {
        let out = rewrite_bindings("", &section(2, "SDL/0/danstick Player 2"));
    ...
    fn the_keyboard_takes_the_first_free_port_in_dolphins_own_keys() {
        let text = sections(&[1], None, |p| format!("danstick Player {p}"));
    ...
    fn a_users_other_dsu_servers_are_kept() {
        ...
        assert_eq!(
            get_ini(&text, "Server", "Entries").as_deref(),
            Some("phone:192.168.1.5:26760;danstick:127.0.0.1:26760;")
        );
        assert_eq!(get_ini(&text, "Server", "Enabled").as_deref(), Some("True"));
    }

    #[test]
    fn writing_twice_does_not_list_danstick_twice() {
