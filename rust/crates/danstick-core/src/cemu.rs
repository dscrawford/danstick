...
/// Marks a keyboard profile as danstick's, so a stale one can be told from the user's.
pub const KEYBOARD_MARKER: &str = "<!-- danstick keyboard: the first port no pad holds -->";
...
/// danstick's keyboard layout on the Wii U's controls, by Nintendo label:
...
/// Whether a profile on disk is danstick's keyboard, as opposed to one the user made.
