...
use padmap_core::{guess, icons, layout, profile, retroarch, scope, sdl, standard};
...
fn bindings_from(raw: &Value) -> BTreeMap<Control, Binding> {
    raw.as_object()
        ...
        .iter()
        .map(|(name, value)| (name.parse::<Control>().expect("a recorded control name"), binding_from(value)))
        .collect()
}

fn u16s(raw: &Value) -> Vec<u16> {
    raw.as_array().expect("an array of codes").iter().map(|v| v.as_u64().expect("a code") as u16).collect()
}
...
fn strings(raw: &Value) -> Vec<String> {
    raw.as_array().expect("an array").iter().map(|v| v.as_str().expect("a string").to_owned()).collect()
}
...
fn pairs(raw: &Value) -> Vec<(String, String)> {
    raw.as_array()
        .expect("pairs")
        .iter()
        .map(|pair| (pair[0].as_str().expect("field").to_owned(), pair[1].as_str().expect("target").to_owned()))
        .collect()
}
...
fn expect_fallible(recorded: &Value, actual: Result<String, impl std::fmt::Display>, what: &str) {
    ...
    match (ok, actual) {
        (true, Ok(value)) => assert_eq!(value, recorded["value"].as_str().expect("a value"), "{what}"),
        (false, Err(_)) => {}
        (true, Err(error)) => panic!("{what}: the Python answered {:?}, this refused ({error})", recorded["value"]),
        (false, Ok(value)) => panic!("{what}: the Python refused ({}), this answered {value:?}", recorded["error"]),
    }
}
...
fn bindings_spell_the_same_thing_to_both_consumers() {
    ...
    for case in &cases {
        ...
        assert_eq!(binding.retroarch_visible(), out["retroarch_visible"], "retroarch_visible for {what}");
...
fn both_consumers_number_buttons_the_way_the_python_did() {
    for case in corpus("button_indices") {
        ...
        assert_eq!(retroarch_button_index(&keys, code), expected_ra, "retroarch {code:#x} in {keys:x?}");
    }
}
...
fn a_hostile_device_name_is_cleaned_the_same_way() {
    for case in corpus("sdl_lines") {
        ...
        let built = sdl::line(str_at(input, "guid"), str_at(input, "name"), &fields, str_at(input, "platform"));
...
fn a_capture_becomes_the_same_sdl_line() {
    for case in corpus("sdl_mappings") {
        ...
        let sticks = input["sticks"].is_object().then(|| fields_from(&input["sticks"]));
    ...
    for case in corpus("parsed_sdl_lines") {
        ...
        match (sdl::parse_line(raw), case["out"].as_object()) {
            ...
            (Some((guid, name, fields)), Some(expected)) => {
                ...
                assert_eq!(ordered(&fields), pairs(&expected["fields"]), "{raw:?} -- field order is part of the answer");
            }
            (parsed, expected) => panic!("{raw:?}: the Python said {expected:?}, this said {parsed:?}"),
        }
    }
}
        ...
        let axes: Option<BTreeMap<u16, AxisSpan>> = input["axes"].as_array().map(|entries| {
            entries
                ...
                .map(|entry| {
                    ...
                    (entry[0].as_u64().expect("a code") as u16, AxisSpan::new(at(0), at(1), at(2)))
                })
                .collect()
        });
...
fn a_capture_becomes_the_same_autoconfig() {
    for case in corpus("retroarch_lines") {
        ...
        let overrides: BTreeMap<Control, String> = input["overrides"]
            ...
            .expect("overrides")
            .iter()
            .map(|(name, value)| (name.parse::<Control>().expect("a control name"), value.as_str().expect("a key").to_owned()))
            .collect();
        assert_eq!(retroarch::lines(&bindings, &overrides), strings(&case["out"]), "for {input}");
    }
}
...
fn the_shadowed_axis_rule_drops_the_same_lines() {
    for case in corpus("shadowed_axis_halves") {
        ...
        assert_eq!(retroarch::drop_shadowed_axis_halves(input.clone()), strings(&case["out"]), "for {input:?}");
    }
}
...
fn every_layout_carries_the_same_coordinates_labels_and_overrides() {
    ...
    for case in corpus("layouts") {
        ...
        for (shape, wanted) in layout.shapes.iter().zip(shapes) {
            ...
            let points: Vec<f64> =
                wanted["points"].as_array().expect("points").iter().map(|v| v.as_f64().expect("a coordinate")).collect();
        ...
        for (control, wanted) in layout.controls.iter().zip(controls) {
            ...
            assert_eq!(control.retroarch, str_at(wanted, "retroarch"), "{id}/{name}: the console's own key wiring");
        }
    }
}
...
fn the_corpus_covers_every_layout_the_port_ships() {
    let recorded: BTreeSet<String> = corpus("layouts").iter().map(|case| str_at(case, "in").to_owned()).collect();
    let shipped: BTreeSet<String> = layout::all().iter().map(|layout| layout.id.clone()).collect();
...
fn every_codepoint_is_printable_to_the_same_answer_as_python() {
    ...
    for case in &cases {
        ...
        assert_eq!(profile::printable(character), case["out"].as_bool().expect("a verdict"), "U+{point:04X}");
    }
}
...
fn a_controller_name_becomes_the_same_signature_and_the_same_filename() {
    for case in corpus("signatures") {
        let input = &case["in"];
        let signature = profile::signature(u16_at(input, "vid"), u16_at(input, "pid"), str_at(input, "name"));
        assert_eq!(signature, str_at(&case["out"], "signature"), "for {input}");
        assert_eq!(profile::filename(&signature), str_at(&case["out"], "filename"), "for {input}");
    }
}
...
fn a_stored_profile_reads_back_and_writes_out_the_way_the_python_did() {
    for case in corpus("stored_profiles") {
        ...
        assert_eq!(profile.has_bindings(), expected["has_bindings"], "for {input}");
        ...
        assert_eq!(profile.to_value(), expected["json"], "written form, for {input}");
    }
}
...
fn every_controller_name_gets_the_same_icon_as_the_python() {
    ...
    for case in &cases {
        let input = &case["in"];
        let got = icons::for_pad(u16_at(input, "vid"), u16_at(input, "pid"), str_at(input, "name"), None, &BTreeMap::new());
...
fn the_udev_rules_match_the_python_rule_for_rule() {
    ...
    fn effective(rules: &str) -> Vec<&str> {
        rules.lines().map(str::trim_end).filter(|line| !line.is_empty() && !line.starts_with('#')).collect()
    }
    for case in corpus("hide_rules") {
        let pads: Vec<Hideable> = case["in"]
            ...
            .expect("pads")
            .iter()
            .map(|raw| Hideable { name: str_at(raw, "name").to_owned(), vid: u16_at(raw, "vid"), pid: u16_at(raw, "pid") })
            .collect();
        let out = &case["out"];
        let mine = hide::generate_rules(&pads);
        let theirs = str_at(out, "rules");
        assert_eq!(effective(&mine), effective(theirs), "rules for {:?}", case["in"]);
        assert_eq!(hide::covered(&mine), hide::covered(theirs), "coverage");
        assert_eq!(hide::nix_module_snippet(&pads), str_at(out, "nix"), "nix snippet for {:?}", case["in"]);
        let wanted: Vec<(u16, u16)> =
            out["targets"].as_array().expect("targets").iter().map(|raw| (u16_at(raw, "vid"), u16_at(raw, "pid"))).collect();
        let got: Vec<(u16, u16)> = hide::targets(&pads, &[]).iter().map(|p| (p.vid, p.pid)).collect();
        assert_eq!(got, wanted, "targets for {:?}", case["in"]);
    }
}

/// padmap's own identity, which is what the corpus was recorded under.
const PADMAP_IDENTITY: Identity = Identity { bustype: 0x06, vendor: 0x1209, product: 0x0001, version: 0x0001 };
...
fn identity_for(player: u32) -> Identity {
    Identity { version: emit::version_for(player), ..PADMAP_IDENTITY }
}
...
fn a_virtual_pads_guid_is_the_one_the_python_computed() {
    for case in corpus("virtual_guids") {
        ...
        assert_eq!(emit::virtual_guid(player, identity_for(player)), str_at(&case, "out"), "player {player}");
    }
}
...
fn the_python_guessed_a_standard_pad_wrongly_and_padmap_does_not() {
    ...
    let keys: Vec<u16> = vec![0x130, 0x131, 0x133, 0x134, 0x136, 0x137, 0x13A, 0x13B, 0x13C, 0x13D, 0x13E];
    ...
    assert_eq!(theirs[6], ("lefttrigger", "b6"), "BTN_SELECT is at index 6 and is back");
