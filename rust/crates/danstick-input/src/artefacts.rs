...
pub const ENV_SDL_DB: &str = "DANSTICK_SDL_DB";

pub fn sdl_database_path() -> PathBuf {
    env_path(ENV_SDL_DB)
        .unwrap_or_else(|| config_dir().join("danstick").join("sdl_controllers.txt"))
}
...
/// Replace danstick's lines, keeping every other; refuses an existing file it cannot read.
...
pub fn cemu_profile_dir() -> PathBuf {
    env_path("DANSTICK_CEMU_DIR")
        .unwrap_or_else(|| config_home().join("Cemu").join("controllerProfiles"))
}

/// Up to Cemu's eight slots; profiles for players danstick is not managing are left alone.
...
pub fn dolphin_config_dir() -> PathBuf {
    env_path("DANSTICK_DOLPHIN_DIR").unwrap_or_else(|| config_home().join("dolphin-emu"))
}
...
/// Replaces only `input_config`, and only danstick's players within it.
pub fn write_ryujinx_config(
    ...
    let merged = danstick_core::ryujinx::merge(
        config
            .get("input_config")
...
pub fn carried_fields(guid: &str) -> Option<(danstick_core::fields::Fields, String)> {
    if guid.is_empty() {
        return None;
    }
    for path in sdl_database_paths() {
        ...
        for line in text.lines() {
            let Some((found, name, fields)) = danstick_core::sdl::parse_line(line) else {
                continue;
            };
...
pub fn binding_fields(fields: &danstick_core::fields::Fields) -> danstick_core::fields::Fields {
    let mut out = danstick_core::fields::Fields::new();
...
mod tests {
    ...
    fn scratch(tag: &str) -> PathBuf {
        let dir = std::env::temp_dir().join(format!("danstick-artefact-{tag}"));
    ...
    fn a_carried_line_drops_the_identity_fields_and_skips_our_own() {
        let mut fields = danstick_core::fields::Fields::new();
        fields.insert("a", "b0");
    ...
    fn a_first_run_with_no_database_still_writes_one() {
        ...
        let written = write_sdl_database(&one_line(), &BTreeMap::new(), |_| DANSTICK, Some(&path))
    ...
    fn a_hand_written_line_survives_a_rewrite() {
        ...
        write_sdl_database(&one_line(), &BTreeMap::new(), |_| DANSTICK, Some(&path))
            .expect("write");
        ...
        assert!(text.contains("danstick Player 1"));
        let _ = std::fs::remove_dir_all(&dir);
    }
    ...
    fn an_unreadable_database_is_refused_rather_than_overwritten() {
        ...
        let result = write_sdl_database(&one_line(), &BTreeMap::new(), |_| DANSTICK, Some(&path));
        assert!(
