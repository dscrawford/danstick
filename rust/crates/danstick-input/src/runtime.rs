//! Where danstick keeps state that lasts a login session.
...
/// `$XDG_RUNTIME_DIR/danstick`, falling back to `/tmp/danstick`.
pub fn dir() -> PathBuf {
...
pub fn dir_under(base: Option<&str>) -> PathBuf {
    PathBuf::from(base.unwrap_or("/tmp")).join("danstick")
}
...
pub fn icon_overrides_path() -> PathBuf {
    config_home().join("danstick").join("icons.json")
}

pub fn load_icon_overrides() -> BTreeMap<String, String> {
    std::fs::read_to_string(icon_overrides_path())
        .map(|text| danstick_core::icons::parse_overrides(&text))
        .unwrap_or_default()
}
...
fn build_id_from(label: &Path, newest: Option<u128>) -> String {
    if let Some(store) = env_nonempty("DANSTICK_BUILD_ID") {
        return store;
    }
    match newest {
...
/// Identity of the code this binary is running; `DANSTICK_BUILD_ID` (the Nix store path) wins.
...
/// Identity of a Python source tree: the newest `.py` mtime, unless `DANSTICK_BUILD_ID` is set.
...
pub fn is_daemon_argv(argv: &[String]) -> bool {
    ...
    let program = Path::new(before)
        ...
        .unwrap_or("");
    program == "danstick" || program == "danstick-rs"
}

/// Pids of danstick daemons serving a given `XDG_RUNTIME_DIR` (which decides the socket).
...
/// Move what padmap, as this project was called, kept under `$XDG_DATA_HOME`
/// and `$XDG_CONFIG_HOME` to danstick's names, so nobody sets a pad up twice.
pub fn adopt_padmap_dirs() {
    let data = env_path("XDG_DATA_HOME").unwrap_or_else(|| home().join(".local").join("share"));
    for base in [data, config_home()] {
        adopt(&base.join("padmap"), &base.join("danstick"));
    }
}

/// Rename `old` to `new` when only `old` exists; whether it moved.
pub fn adopt(old: &Path, new: &Path) -> bool {
    if new.exists() || !old.is_dir() {
        return false;
    }
    match std::fs::rename(old, new) {
        Ok(()) => {
            log::info!("moved {} to {}", old.display(), new.display());
            true
        }
        Err(error) => {
            log::warn!(
...
mod tests {
    ...
    fn a_daemon_is_recognised_by_its_argv_shape_not_a_substring() {
        ...
        assert!(is_daemon_argv(&owned(&[
            "/nix/store/x/bin/danstick",
            "serve"
        ])));
        assert!(is_daemon_argv(&owned(&["danstick-rs", "serve"])));
        assert!(is_daemon_argv(&owned(&[
            "python3",
            "-m",
            "danstick.cli",
        ...
        assert!(!is_daemon_argv(&owned(&["bash", "-c", "danstick serve"])));
        assert!(!is_daemon_argv(&owned(&["danstick", "list"])));
        assert!(!is_daemon_argv(&owned(&["danstick.cli", "serve"])), "no -m");
    ...
    fn the_prompted_file_round_trips_and_survives_bytes_that_are_not_utf8() {
        let dir = std::env::temp_dir().join(format!("danstick-prompted-{}", std::process::id()));
    ...
    fn the_base_comes_from_the_environment_but_defaults_to_tmp() {
        assert_eq!(
            ...
            PathBuf::from("/run/user/1000/danstick")
        );
        assert_eq!(dir_under(None), PathBuf::from("/tmp/danstick"));
    }

    #[test]
    fn padmaps_directory_moves_once_and_never_over_danstick() {
        let root = std::env::temp_dir().join(format!("danstick-adopt-{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&root);
        let (old, new) = (root.join("padmap"), root.join("danstick"));
        std::fs::create_dir_all(old.join("devices")).expect("mkdir");
        assert!(adopt(&old, &new));
        assert!(new.join("devices").is_dir() && !old.exists());
        std::fs::create_dir_all(&old).expect("mkdir");
        assert!(
            !adopt(&old, &new),
            "danstick's own directory is never replaced"
        );
        assert!(old.exists());
        let _ = std::fs::remove_dir_all(&root);
    }
