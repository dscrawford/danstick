...
pub fn keyboard_assignment(player: u32) -> Assignment {
    Assignment {
        ...
        name: danstick_core::keyboard::SEAT_NAME.to_owned(),
        phys: String::new(),
        vid: 0,
        pid: 0,
    }
...
mod tests {
    ...
    fn a_missing_file_is_an_empty_order_rather_than_a_failure() {
        let path = Path::new("/nonexistent-danstick-test/assignments.json");
    ...
    fn a_saved_order_reads_back_as_itself() {
        let dir = std::env::temp_dir().join("danstick-assignment-test");
