...
use danstick_core::profile::{self, Profile};
use log::warn;

use crate::pad::Pad;
use crate::runtime::{env_path, home};

pub const ENV_DIR: &str = "DANSTICK_PROFILE_DIR";
...
mod tests {
    ...
    fn scratch(tag: &str) -> PathBuf {
        let dir = std::env::temp_dir().join(format!("danstick-profile-test-{tag}"));
    ...
    fn the_override_wins_over_the_xdg_path() {
        ...
        std::env::set_var(ENV_DIR, "/tmp/danstick-override");
        assert_eq!(dir(), PathBuf::from("/tmp/danstick-override"));
        match previous {
