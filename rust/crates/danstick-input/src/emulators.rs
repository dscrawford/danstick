...
/// VALUE for CONFIG_ENV variable (for `danstick-rs exec`).
...
mod tests {
    ...
    fn scratch(tag: &str) -> PathBuf {
        let dir = std::env::temp_dir().join(format!("danstick-emulators-{tag}"));
    ...
    fn the_env_file_carries_every_mapping_and_is_sourceable() {
        ...
        assert!(text.contains("danstick Player 1") && text.contains("danstick Player 2"));
    ...
    fn a_pad_with_no_mapping_contributes_no_line() {
        ...
        assert_eq!(value, "guid1,danstick Player 1,a:b0,");
    }
    ...
    fn the_keyboard_takes_the_first_free_port_everywhere_and_nowhere_else() {
        ...
        std::fs::write(
            ...
            danstick_core::cemu::keyboard_profile(3),
        )
        .expect("stale");
