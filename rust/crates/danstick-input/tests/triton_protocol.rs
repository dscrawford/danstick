...
fn the_probe_is_gated_by_the_switch_that_protects_real_controllers() {
    ...
    assert!(danstick_input::pad::wanted_by("Steam Controller", None));
    assert!(danstick_input::pad::wanted_by("Steam Controller", Some("")));
    assert!(danstick_input::pad::wanted_by(
        "Steam Controller",
        Some("Steam")
    ));
    assert!(!danstick_input::pad::wanted_by(
...
fn the_packet_matches_the_python_byte_for_byte() {
    ...
    let python = std::process::Command::new("python3")
        .args([
            "-c",
            "import sys; sys.path.insert(0,'src'); from danstick import triton; \
    ...
    if !output.status.success() {
        eprintln!("skipping: python could not import danstick.triton");
        return;
    }
...
fn only_valve_ids_on_a_hidraw_node_are_ours() {
    let ours = danstick_input::pad::Pad {
    ...
    let evdev = danstick_input::pad::Pad {
    ...
    let other = danstick_input::pad::Pad {
        vid: 0x057E,
        pid: 0x2009,
        ..ours.clone()
    };
...
fn a_regular_file_is_refused_as_a_hidraw_node() {
    let path = std::env::temp_dir().join("danstick-triton-not-a-device");
...
fn a_missing_node_is_an_error_not_a_panic() {
    let missing = PathBuf::from("/nonexistent-danstick-triton/hidraw0");
...
fn a_press_on_a_puck_claims_a_seat() {
    use danstick_core::assign::{Assigner, HOLD_SECONDS};
